...
##n=100
#ica: 0.9686586, 0.9382996
start_time<- Sys.time()
print(start_time)
print("n100 high newparameter")
highmetricn100<-matrix(c(0.4, 0.3937989, -0.5, -0.4, 0.3872117, -0.5), ncol = 6)
...
for (N in 1:1000) {
  ...
  for(m in 1:1000) {
    ...
    Sigma_matrix[1,2]<-Sigma_matrix[2,1]<- highmetricn100[,1]*sd(T0)*sd(T1)
    Sigma_matrix[1,4]<-Sigma_matrix[4,1]<- highmetricn100[,3]*sd(T0)*sd(S1)
    Sigma_matrix[2,3]<-Sigma_matrix[3,2]<- highmetricn100[,4]*sd(T1)*sd(S0)
    Sigma_matrix[3,4]<-Sigma_matrix[4,3]<- highmetricn100[,6]*sd(S0)*sd(S1)
    
    if(is.positive.definite(Sigma_matrix)==TRUE){
      
      payhigh<-(sqrt(vT0T0*vS0S0)*cT0S0)+(sqrt(vT1T1*vS1S1)*cT1S1)-(sqrt(vT1T1*vS0S0)*highmetricn100[,4])-(sqrt(vT0T0*vS1S1)*highmetricn100[,3])
      paydahigh<-sqrt((vT0T0+vT1T1-2*sqrt(vT0T0*vT1T1)*highmetricn100[,1])*(vS0S0+vS1S1-2*sqrt(vS0S0*vS1S1)*highmetricn100[,6]))
      ...
      sigmadelta_shigh<-var(S0,S0)+var(S1,S1)-2*highmetricn100[,6]*sqrt(var(S0,S0)*var(S1,S1))
      sigmadeltashigh<- append(sigmadelta_shigh,sigmadeltashigh)
      sigmadelta_thigh<-var(T0,T0)+var(T1,T1)-2*highmetricn100[,1]*sqrt(var(T0,T0)*var(T1,T1))
      sigmadeltathigh<- append(sigmadelta_thigh,sigmadeltathigh)
      cormatrixhigh<- matrix(c(1, highmetricn100[,1], cT0S0,highmetricn100[,3],
                               highmetricn100[,1],1,highmetricn100[,4],cT1S1,
                               cT0S0,highmetricn100[,4],1,highmetricn100[,6],
                               highmetricn100[,3],cT1S1,highmetricn100[,6], 1 ), nrow = 4)
